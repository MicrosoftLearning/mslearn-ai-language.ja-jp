...
1. **Main** 関数で、既存のコードが **articles** フォルダー内のすべてのファイルを読み取り、その内容を含む一覧を作成することに注意してください。 C# コードの場合、**TextDocumentInput** オブジェクトの一覧を使用して、ファイル名を ID と言語として含めます。 Python では、テキスト コンテンツの単純な一覧が使用されます。
